...
{
//victor	unsigned int cif_vol_sel;
#if 0
...
static irqreturn_t camsys_cif_irq(int irq,  void *data)
{
    ...
    intsta = __raw_readl((char *)(camsys_dev->devmems.registermem->vir_base +
		CIF_INITSTA));
    frmsta = __raw_readl((char *)(camsys_dev->devmems.registermem->vir_base +
		CIF_FRAME_STATUS));
   printk("get oneframe, intsta = 0x%x \n", intsta);

    if (intsta & 0x200) {
		__raw_writel(0x200,
			(char *)(camsys_dev->devmems.registermem->vir_base +
			CIF_INITSTA));
		__raw_writel(0xf000,
			(char *)(camsys_dev->devmems.registermem->vir_base +
			CIF_CTRL));
